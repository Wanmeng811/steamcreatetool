...
一个基于 PyQt5 的桌面应用，用于半自动生成和管理 Steam 账号。
本工具依赖 CloudMail (https://github.com/Yuyuyang/cloudmail) 邮件服务系统，通过其 REST API 创建临时邮箱并监听邮件。

请注意：本工具不会自动填写 Steam 注册表单，您需要手动将生成的用户名和密码填入注册页面，工具只负责：
- 生成随机账号信息
- 创建临时邮箱
- 自动点击验证链接
- 提取并显示 5 位验证码
...
- 邮件监听与验证码提取
  ...
  若邮件在 5 分钟内到达，自动提取验证链接并点击，同时捕获 5 位验证码显示在界面中。
...
安装依赖
在终端中执行以下命令安装所需 Python 包：

    pip install PyQt5 requests

启动程序
将代码保存为 main.py，然后在终端中运行：
...
提示：如果您使用 CloudMail 的默认配置，请参考其部署文档 (https://github.com/Yuyuyang/cloudmail) 设置好管理员账号和域名，确保 API 可正常访问。
...
致谢
----
邮件服务基于 CloudMail (https://github.com/Yuyuyang/cloudmail) 开源项目，感谢作者 Yuyuyang 的优秀解决方案。
